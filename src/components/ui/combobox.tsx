    ...
    const renderItem = React.useCallback(
      ({ item }: ListRenderItemInfo<unknown>) => {
        ...
        return (
          <TouchableOpacity
            // variant="ghost"
            className="flex-row android:flex-1 justify-between py-4"
            style={{ minHeight: 50 }}
            onPress={() => {
          ...
          >
            <View className="flex-row justify-between">
              <View className="flex-row gap-2">
                ...
                <Text className={"text-foreground text-lg "}>
                  {listItem.currency ? listItem.currency : listItem.countryCode}
                </Text>
              </View>
            </View>
            {isSelected && <Check size={30} className={"text-info mt-1.5"} />}
          </TouchableOpacity>
        );
      },
      [selectedItem, selectedItemProp],
