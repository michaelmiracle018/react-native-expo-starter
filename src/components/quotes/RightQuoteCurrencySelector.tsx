...
export default function RightQuoteCurrencySelector() {
  const { quoteTargetCurrency, selectQuoteTradeType } = useQuoteSelector();
  const { setQuoteTargetCurrency } = useQuoteSelectorControls();
  ...
  React.useEffect(() => {
    ...
    setQuoteTargetCurrency(newData);
  }, []);

  return (
    <View className="flex-row gap-1 items-center">
      <Text className="font-bold text-lg ">
        {selectQuoteTradeType === "Buy" ? t(i18n)`With` : t(i18n)`For`}
      </Text>
      <MarketSelect
        selectedValue={quoteTargetCurrency}
        onValueChange={(value: any) => {
          setQuoteTargetCurrency(value);
        }}
        canUnSelect={false}
