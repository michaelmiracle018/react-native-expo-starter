...
import { increment } from "~/reduxStore/features/counterSlice";
...
export const setAuthTokens = (tokens: AuthTokens): Promise<void> =>
  ...
  storeDataInStorage(STORAGE_KEY, tokens);
...
export const clearAuthTokens = (): Promise<void> =>
  ...
  removeDataFromStorage(STORAGE_KEY);
...
export const applyAuthTokenInterceptor = (
  ...
  axios.interceptors.request.use(authTokenInterceptor(config) as any);
};
...
const getAuthTokens = async (): Promise<AuthTokens | undefined> => {
  const rawTokens = await getDataFromStorage(STORAGE_KEY);
