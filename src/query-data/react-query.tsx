...
import { isNative } from "~/lib/platform/detection";
import CryptoJS from "react-native-crypto-js";
import {
  LABELERS_DETAILED_INFO_QUERY_KEY_ROOT,
  REACT_QUERY_KEY,
} from "~/api/common/secretKeys";

// any query keys in this array will be persisted to AsyncStorage
export const labelersDetailedInfoQueryKeyRoot =
  LABELERS_DETAILED_INFO_QUERY_KEY_ROOT;
const STORED_CACHE_QUERY_KEY_ROOTS = [labelersDetailedInfoQueryKeyRoot];
const SECRET_KEY = REACT_QUERY_KEY; // Use a strong secret key for encryption
...
function QueryProviderInner({
  ...
  const [persistOptions, _setPersistOptions] = useState(() => {
    const asyncPersister = createAsyncStoragePersister({
      ...
      key: `queryClient-${buildType}-` + (currentDid ?? "logged-out"),
